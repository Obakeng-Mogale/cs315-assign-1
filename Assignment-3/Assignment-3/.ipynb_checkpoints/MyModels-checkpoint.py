...
class Kmeans:
    ...
    def fit(self, X, initial_centers=None, max_iter=1000, tol=1e-4):
        self.X = X
        self.d, self.N = X.shape
        
        # --- NEW LOGIC FOR MANUAL INITIALIZATION ---
        if initial_centers is not None:
            initial_centers = np.array(initial_centers)
            # Make sure the user passed an array of shape (features, clusters) -> (d, k)
            if initial_centers.shape != (self.d, self.num_clusters):
                raise ValueError(f"initial_centers must have shape ({self.d}, {self.num_clusters})")
            
            self.cluster_centers_ = initial_centers.copy()
            
        else:
            # Fallback to your original initialization logic
